...
(* Non-vacuity control for ideal zero.  The same adversary used to distinguish
   the insecure real KDF actually executes one history query, one constrained
   history query, and one live challenge in the bit-free ideal oracle.  The
   live response is the test sampler's negative material, so the adversary is
   eligible and guesses false; zero advantage is therefore caused by bit
   independence, not by skipping the challenge path or forcing ineligibility. *)
