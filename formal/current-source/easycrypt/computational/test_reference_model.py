...
class T(unittest.TestCase):
 def setUp(self):
  ...
  self.f=tuple(fs); self.auth=norm(self.f,self.a,self.k); self.ids=tuple(x.id for x in self.f); self.v=View(self.f,self.ids); self.s=State(self.a,"doc",frozenset({"n"}),{"n":frozenset(self.ids)},{f.id:replace(f,sig=None) for f in self.f})
 ...
 def test_04_revoked_blocks(self):
  r=self.fact("f8",FKind.CR,self.ids,obs=("be",)); fs=(*self.f,r); a=norm(fs,self.a,self.k); v=View(fs,tuple(x.id for x in fs)); s=replace(self.s,nodes=frozenset({"nr"}),closures={"nr":frozenset(v.observed)},fact_contents={**self.s.fact_contents,r.id:replace(r,sig=None)}); o=signop(self.env(preds=("nr",),adigest=a.digest()),"b",self.k); self.assertEqual(validate(o,v,s,self.k).failed,"required-capability-active")
 ...
 def test_07_no_incarnation(self):
  b2=Principal("b","b2"); r=self.fact("f8",FKind.MR,self.ids,obs=("mb",)); x=(*self.ids,"f8"); g=self.fact("f9",FKind.MG,x,b2,None,"mb2"); x=(*x,"f9"); h=self.fact("f10",FKind.CG,x,b2,Cap.EDIT,"be2"); fs=(*self.f,r,g,h); a=norm(fs,self.a,self.k); v=View(fs,tuple(z.id for z in fs)); s=replace(self.s,nodes=frozenset({"n2"}),closures={"n2":frozenset(v.observed)},fact_contents={**self.s.fact_contents,r.id:replace(r,sig=None),g.id:replace(g,sig=None),h.id:replace(h,sig=None)}); o=signop(self.env(preds=("n2",),adigest=a.digest()),"b",self.k); self.mut(o,replace(Cfg(),incarnation=False),v,s)
 ...
 def test_12a_same_id_content_substitution_rejected(self):
  q=self.fact("f8",FKind.MG,self.ids,self.c,None,"mc"); fs=(*self.f,q); a=norm(fs,self.a,self.k); v=View(fs,tuple(x.id for x in fs)); s=replace(self.s,nodes=frozenset({"nx"}),closures={"nx":frozenset(v.observed)},fact_contents={**self.s.fact_contents,q.id:replace(q,sig=None)}); substitute=self.fact("f8",FKind.CG,self.ids,self.c,Cap.EDIT,"ce"); forged_view=View((*self.f,substitute),v.observed); o=signop(self.env(preds=("nx",),author=self.c,adigest=a.digest()),"c",self.k); self.assertEqual(validate(o,forged_view,s,self.k).failed,"exact-author-context")
 def test_13_no_auth_digest(self):
  q=self.fact("f8",FKind.MG,self.ids,self.c,None,"mc"); fs=(*self.f,q); v=View(fs,tuple(x.id for x in fs)); s=replace(self.s,nodes=frozenset({"nx"}),closures={"nx":frozenset(v.observed)},fact_contents={**self.s.fact_contents,q.id:replace(q,sig=None)}); self.mut(signop(self.env(preds=("nx",)),"b",self.k),replace(Cfg(),digest=False),v,s)
 def test_14_no_predecessor_complete(self):
  q=self.fact("f8",FKind.MG,self.ids,self.c,None,"mc"); s=replace(self.s,nodes=frozenset({"nx"}),closures={"nx":frozenset((*self.ids,q.id))},fact_contents={**self.s.fact_contents,q.id:replace(q,sig=None)}); self.mut(signop(self.env(preds=("nx",)),"b",self.k),replace(Cfg(),complete=False),self.v,s)
